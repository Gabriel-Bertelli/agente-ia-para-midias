...
import { Database, Loader2, AlertCircle, AlertTriangle, LogOut, LayoutDashboard, Filter, Calendar, Tag, Box, TrendingUp, Sparkles, Download, Table as TableIcon, Lock, Eye, EyeOff, GraduationCap, Megaphone, Target, Bell } from 'lucide-react';
import { BarChart, Bar, Legend, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LabelList, LineChart, Line, ComposedChart, Cell } from 'recharts';
...
function Dashboard({ data }: { data: any[] }) {
  ...
  // Share chart state
  const [shareKpi, setShareKpi] = useState(numericFields[0] || '');
  const [shareDimension, setShareDimension] = useState('tipo_campanha');
  ...
  const efficiencyByCampaignData = useMemo(() => {    if (!dateField || !campaignField || selectedEfficiencyCampaigns.length === 0) return [];
  ...
  const STACK_COLORS = ['#00e5a0', '#818cf8', '#fbbf24', '#f87171', '#06b6d4', '#f472b6', '#34d399', '#a78bfa', '#60a5fa', '#fb923c'];
  const LINE_COLORS = ['#00e5a0', '#818cf8', '#fbbf24', '#f87171', '#06b6d4', '#f472b6', '#34d399', '#fb923c'];
  const SHARE_COLORS = ['#3b82f6', '#f87171', '#fbbf24', '#00e5a0', '#818cf8', '#06b6d4', '#f472b6', '#fb923c', '#34d399', '#a78bfa', '#e879f9', '#4ade80'];

  // Share / representativeness chart data — 100% stacked bar by date
  const { shareChartData, shareKeys } = useMemo(() => {
    if (!shareKpi || !shareDimension || !dateField) return { shareChartData: [], shareKeys: [] };

    const dimCol = getDimensionColumn(shareDimension, shareKpi);

    // Group by date + dimension
    const grouped: Record<string, Record<string, number>> = {};
    filteredData.forEach(row => {
      let dateKey = String(row[dateField] || '');
      try {
        const d = parseLocalDate(dateKey);
        if (isValid(d)) {
          if (timeGrouping === 'monthly') dateKey = format(d, 'yyyy-MM');
          else if (timeGrouping === 'weekly') {
            const ws = new Date(d); ws.setDate(d.getDate() - d.getDay());
            dateKey = format(ws, 'yyyy-MM-dd');
          } else dateKey = format(d, 'yyyy-MM-dd');
        }
      } catch (_) {}

      const dim = String(row[dimCol] || 'Outros');
      if (!grouped[dateKey]) grouped[dateKey] = {};
      grouped[dateKey][dim] = (grouped[dateKey][dim] || 0) + (Number(row[shareKpi]) || 0);
    });

    // Collect all dimension keys, sort by total desc, group small ones into "Outros"
    const allDims: Record<string, number> = {};
    Object.values(grouped).forEach(bucket => {
      Object.entries(bucket).forEach(([k, v]) => { allDims[k] = (allDims[k] || 0) + v; });
    });
    const sortedDims = Object.entries(allDims).sort((a, b) => b[1] - a[1]);
    const TOP_N = 6;
    const topDims = sortedDims.slice(0, TOP_N).map(([k]) => k);
    const hasOthers = sortedDims.length > TOP_N;
    const finalKeys = hasOthers ? [...topDims, 'Outros (menores)'] : topDims;

    // Build 100% normalized rows
    const rows = Object.entries(grouped)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([dateKey, bucket]) => {
        // aggregate others
        const merged: Record<string, number> = {};
        topDims.forEach(k => { merged[k] = bucket[k] || 0; });
        if (hasOthers) {
          merged['Outros (menores)'] = Object.entries(bucket)
            .filter(([k]) => !topDims.includes(k))
            .reduce((s, [, v]) => s + v, 0);
        }
        ...
        finalKeys.forEach(k => {
          pctRow[k] = total > 0 ? parseFloat(((merged[k] || 0) / total * 100).toFixed(1)) : 0;
          pctRow[`_abs_${k}`] = merged[k] || 0;
        });
        return pctRow;
      });

    return { shareChartData: rows, shareKeys: finalKeys };
  }, [filteredData, shareKpi, shareDimension, dateField, timeGrouping]);
    ...
    <div className="space-y-6">
      ...
      {numericFields.length === 0 ? (
        ...
        <>
            ...
            {summaryKpis.map(kpi => {
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border transition-all ${isClickable ? 'cursor-pointer' : ''} ${selectedBarKpi === kpi.id ? 'border-emerald-500 ring-1 ring-emerald-500' : 'border-slate-200 hover:border-slate-200'}`}
                  style={{
                    background: selectedBarKpi === kpi.id ? 'rgba(0,229,160,0.07)' : 'var(--bg-surface)',
                    boxShadow: selectedBarKpi === kpi.id
                      ? '0 0 0 1px rgba(0,229,160,0.25), 0 4px 20px rgba(0,0,0,0.4)'
                      : '0 1px 4px rgba(0,0,0,0.35)',
                  }}
                >
                  <div className="flex items-start justify-between mb-2" style={{ minHeight: '2rem' }}>
                    <div style={{
                      color: '#b0b0cc',
                      fontSize: '10px',
                      fontWeight: 700,
                  ...
                  <div style={{
                    ...
                    {kpi.isCurrency
                      ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', notation: 'compact', maximumFractionDigits: 1 }).format(kpi.value)
                      : kpi.isPercentage
                        ? new Intl.NumberFormat('pt-BR', { maximumFractionDigits: 1 }).format(kpi.value) + '%'
                        : new Intl.NumberFormat('pt-BR', { notation: 'compact', maximumFractionDigits: 1 }).format(kpi.value)}
                  </div>
                  {isClickable && (
                    <div style={{ fontSize: '9px', color: selectedBarKpi === kpi.id ? 'var(--accent)' : '#606080', marginTop: '6px', letterSpacing: '0.04em' }}>
                      {selectedBarKpi === kpi.id ? '● selecionado no gráfico' : '○ clique para selecionar'}
                    </div>
                  )}
                </div>
              );
          ...
          {/* Share / Representativeness Chart */}
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
              <div>
                <h3 className="text-lg font-semibold text-slate-800 flex items-center gap-2">
                  <TrendingUp className="w-5 h-5 text-emerald-600" />
                  Evolução — Share de <span className="text-emerald-600 ml-1">{getDisplayKpiName(shareKpi)}</span>
                  <span className="text-slate-500 font-normal text-sm ml-1">por {shareDimension.replace(/_/g, ' ')}</span>
                  <span style={{ fontSize: '10px', background: 'rgba(0,229,160,0.12)', color: 'var(--accent)', borderRadius: '6px', padding: '2px 8px', fontWeight: 700, letterSpacing: '0.04em' }}>canais menores acumulados em outros</span>
                </h3>
                <p className="text-slate-500 text-xs mt-1">Composição percentual acumulada ao longo do tempo</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <select
                  value={shareDimension}
                  onChange={e => setShareDimension(e.target.value)}
                ...
                <select
                  value={shareKpi}
                  onChange={e => setShareKpi(e.target.value)}
                  className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                >
            ...
            <div className="h-[380px] w-full">
              {shareChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={shareChartData} margin={{ top: 24, right: 12, left: 0, bottom: 30 }} barCategoryGap="28%">
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(255,255,255,0.05)" />
                    <XAxis
                      dataKey="label"
                      axisLine={false}
                      tickLine={false}
                      tick={{ fill: '#a0a0bc', fontSize: 11 }}
                      dy={10}
                      height={40}
                      minTickGap={20}
                    />
                    ...
                    />
                    <Tooltip
                      contentStyle={{ borderRadius: '10px', border: '1px solid rgba(255,255,255,0.12)', background: '#161622', boxShadow: '0 8px 32px rgba(0,0,0,0.7)', color: '#f4f4fa', fontSize: 12 }}
                      formatter={(value: number, name: string, props: any) => {
                        const absKey = `_abs_${name}`;
                        const abs = props.payload?.[absKey];
                        const absFormatted = abs !== undefined ? ` (${new Intl.NumberFormat('pt-BR', { notation: 'compact', maximumFractionDigits: 1 }).format(abs)})` : '';
                        return [`${value.toFixed(1)}%${absFormatted}`, name];
                      }}
                      labelStyle={{ color: '#a0a0bc', marginBottom: '6px', fontWeight: 600 }}
                    />
                    <Legend
                      wrapperStyle={{ paddingTop: '16px', fontSize: '12px' }}
                      formatter={(value) => <span style={{ color: '#c0c0d8' }}>{value}</span>}
                    />
                    {shareKeys.map((key, idx) => (
                      <Bar key={key} dataKey={key} stackId="share" fill={SHARE_COLORS[idx % SHARE_COLORS.length]} radius={idx === shareKeys.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]}>
                        <LabelList
                          dataKey={key}
                          position="center"
                          formatter={(v: number) => v >= 5 ? `${Math.round(v)}%` : ''}
                          style={{ fill: '#ffffff', fontSize: 11, fontWeight: 800, textShadow: '0 1px 4px rgba(0,0,0,0.9), 0 0 8px rgba(0,0,0,0.8)' }}
                        />
                      </Bar>
                    ))}
                  </BarChart>
              ...
              {comparativeData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={comparativeData} margin={{ top: 20, right: 20, left: 10, bottom: 30 }}>
                    ...
                    />
                    <Tooltip
                      contentStyle={{ borderRadius: '10px', border: '1px solid rgba(255,255,255,0.10)', background: '#18181f', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', color: '#f0f0f5' }}
                      ...
                      labelStyle={{ color: '#a0a0bc', marginBottom: '4px' }}
                    />
                    <Legend
                      formatter={(value) => {
                    ...
                    >
                      <LabelList
                        ...
                        style={{ fill: '#00e5a0', fontSize: 11, fontWeight: 700 }}
                      />
                    </Bar>
                    <Line
                      yAxisId="right"
                    ...
                    >
                      <LabelList
                        ...
                        style={{ fill: '#a5b4fc', fontSize: 11, fontWeight: 700 }}
                      />
                    </Line>
                  </ComposedChart>
                </ResponsiveContainer>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={compositionData} margin={{ top: 10, right: 10, left: 0, bottom: 40 }}>
                    ...
                    <Bar dataKey="value" fill="#00e5a0" radius={[4, 4, 0, 0]}>
                      <LabelList dataKey="label" position="top" style={{ fill: '#c0c0d8', fontSize: 11, fontWeight: 600 }} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                  ...
                  <BarChart data={stackedData} margin={{ top: 10, right: 10, left: 0, bottom: 30 }}>
                    ...
                    {stackedKeys.map((key, index) => (
                      <Bar key={key} dataKey={key} stackId="a" fill={STACK_COLORS[index % STACK_COLORS.length]}>
                        <LabelList dataKey={key} position="center" fill="#ffffff" fontSize={10} fontWeight={700} formatter={(v: number) => v > 0 ? new Intl.NumberFormat('pt-BR', { notation: 'compact' }).format(v) : ''} />
                      </Bar>
                    ))}
                  </BarChart>
                </ResponsiveContainer>
                ...
                <div className="h-[200px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={comparativeData} margin={{ top: 5, right: 5, left: -20, bottom: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                      <XAxis dataKey="label" tick={{ fontSize: 10, fill: '#a0a0bc' }} axisLine={false} tickLine={false} minTickGap={20} interval="preserveStartEnd" />
                      <YAxis tick={{ fontSize: 10, fill: '#a0a0bc' }} axisLine={false} tickLine={false} tickFormatter={(v) => new Intl.NumberFormat('pt-BR', { notation: 'compact' }).format(v)} />
                      <Tooltip
                        ...
                        labelStyle={{ fontSize: 12, color: '#a0a0bc' }}
                        contentStyle={{ borderRadius: '8px', fontSize: 12, border: '1px solid rgba(255,255,255,0.10)', background: '#18181f', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', color: '#f0f0f5' }}
                      />
                      <Line type="monotone" dataKey="current_cpmql" stroke="#818cf8" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={comparativeData} margin={{ top: 5, right: 5, left: -20, bottom: 20 }}>
                      ...
                      <Tooltip
                        ...
                        contentStyle={{ borderRadius: '8px', fontSize: 12, border: '1px solid rgba(255,255,255,0.10)', background: '#18181f', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', color: '#f0f0f5' }}
                      />
                      <Line type="monotone" dataKey="current_cac" stroke="#f87171" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
                ...
                <div className="h-[200px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={comparativeData} margin={{ top: 5, right: 5, left: -20, bottom: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                      <XAxis dataKey="label" tick={{ fontSize: 10, fill: '#a0a0bc' }} axisLine={false} tickLine={false} minTickGap={20} interval="preserveStartEnd" />
                      <YAxis tick={{ fontSize: 10, fill: '#a0a0bc' }} axisLine={false} tickLine={false} tickFormatter={(v) => v + '%'} />
                      <Tooltip
                        ...
                        contentStyle={{ borderRadius: '8px', fontSize: 12, border: '1px solid rgba(255,255,255,0.10)', background: '#18181f', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', color: '#f0f0f5' }}
                      />
                      <Line type="monotone" dataKey="current_conv_ticket_mat" stroke="#fbbf24" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
                    ...
                    {selectedEfficiencyCampaigns.map((campaign, index) => (
                      ...
                      >
                        <LabelList
                          ...
                          style={{ fontSize: 11, fontWeight: 700, fill: '#e2e2f0' }}
                        />
                      </Line>
                    ))}
                  </LineChart>
...
function LoginScreen({ onAuth }: { onAuth: () => void }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 font-sans" style={{background: "var(--bg-base)"}}>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
        <div className="px-8 pt-8 pb-6">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-6" style={{background: "var(--accent-dim)", color: "var(--accent)"}}>
            <Lock className="w-5 h-5" />
          </div>
          <h1 className="text-xl font-bold tracking-tight mb-1" style={{fontFamily: "var(--font-display)", color: "var(--text-primary)"}}>PTSmart</h1>
...
export default function App() {
  ...
  const [view, setView] = useState<'table' | 'dashboard' | 'ai' | 'courses' | 'campaigns' | 'goals' | 'investment' | 'alerts'>('dashboard');
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Coerce numeric strings (in-place, numericKeys computed once — O(n) not O(n²))
      ...
      const slice = allData.slice(0, maxRows);
      if (slice.length > 0) {
        const numericKeys = Object.keys(slice[0]).filter(k => !SKIP_KEY.test(k));
  ...
  if (isConnected) {
    return (
      <div className="min-h-screen p-6 md:p-12 font-sans" style={{background: "var(--bg-base)", color: "var(--text-primary)"}}>
        <div className="max-w-6xl mx-auto space-y-6">
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2" style={{fontFamily: "var(--font-display)", color: 'var(--text-primary)'}}>
                <span style={{ color: 'var(--accent)' }}>
                  <Database className="w-6 h-6" />
                </span>
                PTSmart
                <span style={{ fontSize: '11px', background: 'var(--accent-dim)', color: 'var(--accent)', borderRadius: '6px', padding: '2px 8px', fontWeight: 700, letterSpacing: '0.05em', fontFamily: 'var(--font-body)' }}>v5</span>
              </h1>
              <p className="text-sm mt-1" style={{ color: '#a0a0c0' }}>
                Tabela <span className="font-mono" style={{ color: 'var(--accent)', fontSize: '12px' }}>{tableName}</span>
                <span style={{ margin: '0 8px', opacity: 0.4 }}>·</span>
                <span style={{ color: '#a0a0c0' }}>{data.length.toLocaleString('pt-BR')} registros carregados</span>
              </p>
            </div>
            <div className="flex items-center gap-3">
              <button
              ...
              <button
                ...
                style={{ background: 'var(--bg-elevated)', color: '#a0a0c0', border: '1px solid var(--border-default)' }}
              >
          ...
          <div className="flex gap-1 border-b border-slate-200 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            {([
              ...
              { id: 'table',      icon: TableIcon,        label: 'Dados' },
              { id: 'courses',    icon: GraduationCap,    label: 'Cursos' },
              { id: 'campaigns',  icon: Megaphone,        label: 'Campanhas' },
              { id: 'alerts',     icon: Bell,             label: 'Alertas' },
              { id: 'goals',      icon: Target,           label: 'Metas' },
              { id: 'investment', icon: TrendingUp,       label: 'Investimento' },
              { id: 'ai',         icon: Sparkles,         label: 'Assistente IA' },
            ] as const).map(tab => (
              <button
                key={tab.id}
                onClick={() => setView(tab.id as any)}
                className={`pb-3 px-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-1.5 whitespace-nowrap ${view === tab.id ? 'border-emerald-500 text-emerald-600' : 'border-transparent hover:text-slate-700'}`}
                style={{ color: view === tab.id ? 'var(--accent)' : tab.id === 'alerts' ? '#f59e0b' : '#8888a8' }}
              >
          ...
          {view === 'dashboard' ? (
            ...
            <CampaignAlerts data={data} />
          ) : view === 'goals' ? (
            <GoalsView data={data} />
          ) : view === 'investment' ? (
            <InvestmentSuggestion data={data} />
          ) : view === 'ai' ? (
            ...
            <main className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="bg-slate-50 px-6 py-3 border-t border-slate-200 text-xs text-slate-500 flex justify-between">
                <span>Mostrando {Math.min(500, data.length)} de {data.length} registros (tabela limitada a 500 linhas para performance)</span>
