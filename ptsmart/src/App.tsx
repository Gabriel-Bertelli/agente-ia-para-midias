    ...
    try {
      ...
      while (hasMore && allData.length < maxRows) {
        ...
        if (fetchedData && fetchedData.length > 0) {
          // push is O(n) — avoids re-allocating the entire array on every page
      ...
      // Coerce numeric strings to numbers on load (in-place, single pass per row).
      // Build the skip-set from the first row's keys so the regex only runs once.
