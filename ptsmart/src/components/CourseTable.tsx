...
import { GraduationCap, TrendingUp, TrendingDown, Minus, ChevronUp, ChevronDown, ChevronsUpDown, Filter, X, Loader2 } from 'lucide-react';
...
// ── Aggregation using the dual-field business rule ─────────────────────────
//
...
export function CourseTable({ data }: { data: any[] }) {
  ...
  // ── Async aggregation via Web Worker ──────────────────────────────────────
  const { runCourses, busy: workerBusy } = useAggregationWorker();
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      const result = await runCourses({ data, availableKeys, start, end, tipoCampanha });
      if (result !== null) setRows(result);
      else setRows(aggregateCourses(data, availableKeys, { start, end, tipoCampanha }));
    }, 150);
    return () => { if (debounceRef.current) clearTimeout(debounceRef.current); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data, availableKeys, start, end, tipoCampanha]);
  ...
  const sorted = useMemo(() => {
    let r = rows.filter(row => !search || row.curso.toLowerCase().includes(search.toLowerCase()));
    r = [...r].sort((a, b) => {
