...
import { Megaphone, TrendingUp, TrendingDown, ChevronUp, ChevronDown, ChevronsUpDown, Filter, X, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
import { useDebounce } from '../lib/useDebounce';
import { campaignCache, buildCacheKey, getDataHash, useCacheInvalidation } from '../lib/aggregationCache';
...
function aggregateCampaigns(
  ...
  const cacheKey = buildCacheKey('campaigns', opts);
  const dataHash = getDataHash(data);

  // Check cache first
  if (campaignCache.isValid(cacheKey, dataHash)) {
    const cached = campaignCache.get<CampaignRow[]>(cacheKey);
  ...
  const result = Object.entries(grouped).map(([campanha, g]) => ({
    campanha,
    investimento:  g.inv,
    leads:         g.leads,
    mql:           g.mql,
  ...
  // Store in cache
  campaignCache.set(cacheKey, new Map(result.map(r => [r.campanha, r])), new Map(), null, dataHash);
...
export function CampaignTable({ data }: { data: any[] }) {
  ...
  // Initialize worker
  const { aggregate } = useAggregationWorker();

  // Invalidate cache when raw data changes
  useCacheInvalidation(data, campaignCache);

  // Debounce filters para mejorar performance
  ...
  const debouncedProduto = useDebounce(produto, 300);
  const debouncedSearch = useDebounce(search, 300);

  // Reset page ao mudar filtros
  React.useEffect(() => {
    setPageIndex(0);
  }, [debouncedStart, debouncedEnd, debouncedTipo, debouncedProduto, debouncedSearch]);
  ...
  React.useEffect(() => {
    const aggregationOpts = { start: debouncedStart, end: debouncedEnd, tipoCampanha: debouncedTipo, produto: debouncedProduto };
    const cacheKey = buildCacheKey('campaigns', aggregationOpts);
    const dataHash = getDataHash(data);
    ...
    aggregate(
      ...
      availableKeys,
      aggregationOpts,
      () => aggregateCampaigns(data, availableKeys, aggregationOpts) // Fallback
    )
      .then((result) => {
        setRows(result);
        // Store in cache
      ...
      .catch((error) => {
        ...
        try {
          const result = aggregateCampaigns(data, availableKeys, aggregationOpts);
          setRows(result);
          campaignCache.set(cacheKey, new Map(result.map(r => [r.campanha, r])), new Map(), null, dataHash);
        } catch (fallbackError) {
  ...
  }, [data, availableKeys, debouncedStart, debouncedEnd, debouncedTipo, debouncedProduto, aggregate]);
  ...
  const sorted = useMemo(() => {
    let r = rows.filter(row =>
      !debouncedSearch || row.campanha.toLowerCase().includes(debouncedSearch.toLowerCase())
    );
    return [...r].sort((a, b) => {
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex gap-2 ml-auto flex-wrap items-center">
            {isAggregating && (
              <div className="flex items-center gap-1.5 text-xs text-slate-500 bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5">
        ...
        {sorted.length > ROWS_PER_PAGE && (
          <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-t border-slate-200">
            <div className="text-xs text-slate-500">
              Exibindo <strong>{pageIndex * ROWS_PER_PAGE + 1}</strong> a <strong>{Math.min((pageIndex + 1) * ROWS_PER_PAGE, sorted.length)}</strong> de <strong>{sorted.length}</strong> campanhas
            </div>
