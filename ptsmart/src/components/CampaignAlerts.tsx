...
function buildAlerts(data: any[], availableKeys: string[]): CampaignAlert[] {
  ...
  for (const [campaign, { byDate, tipo, produto, platform }] of grouped) {
    ...
    // Ajuste 1: usar apenas os dias com investimento registrado como denominador,
    // não a janela fixa — campanhas que iniciaram depois dos 60 dias são comparadas
    // corretamente dentro do período em que estiveram ativas.
    const activeDays60 = days60.length;
    const activeDays7  = days7.length;

    const avg7  = activeDays7  > 0 ? totalInv7  / activeDays7  : 0;
    const avg60 = activeDays60 > 0 ? totalInv60 / activeDays60 : 0;

    if (avg60 === 0) continue;

    // Ajuste 2: campanha sem nenhum investimento nos últimos 7 dias = pausada → ignorar
    if (totalInv7 === 0) continue;
      ...
      <div style={{ ...panel, padding: 16 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 12px', borderRadius: 8, background: 'rgba(129,140,248,0.07)', border: '1px solid rgba(129,140,248,0.15)' }}>
          ...
          <p style={{ fontSize: 12, color: '#a0a0bc', lineHeight: 1.5 }}>
            <strong style={{ color: '#c0c0d8' }}>Regra de alerta:</strong> compara a <strong style={{ color: '#c0c0d8' }}>média diária dos últimos 7 dias ativos</strong> com a <strong style={{ color: '#c0c0d8' }}>média diária dos últimos 60 dias ativos</strong> de investimento por campanha.
            Campanhas <strong style={{ color: '#c0c0d8' }}>sem investimento nos últimos 7 dias</strong> são consideradas pausadas e ocultadas.
            O denominador usa apenas os dias com investimento registrado — campanhas recentes não são penalizadas pelos dias anteriores ao seu início.
