...
export function GoalsView({ data }: { data: any[] }) {
  ...
  // Detect unique products in data for filter dropdown
  const produtoOptions = useMemo(() => {
    if (!produtoField) return [] as string[];
    return Array.from(new Set(data.map(d => String(d[produtoField] ?? '').trim()).filter(Boolean))).sort();
  }, [data, produtoField]);

  // Auto-select product when only one is loaded
  React.useEffect(() => {
    if (produtoOptions.length === 1 && filterProduto === 'all') {
      setFilterProduto(produtoOptions[0]);
    }
  }, [produtoOptions]);

  // Filter data and goals by selected product
  ...
  const filteredGoals = useMemo(() => {
    if (filterProduto === 'all') return goals;
    // mapToBU maps produto → BU; goals use BU field directly
    return goals.filter(g => g.bu === filterProduto);
  }, [goals, filterProduto]);
  ...
  const actualMap = useMemo(() => aggregateActual(filteredData, availableKeys, start, end),           [filteredData, availableKeys, start, end]);
  const goalMap   = useMemo(() => aggregateGoals(filteredGoals, start, goalsEnd),                     [filteredGoals, start, goalsEnd]);
  ...
  const dailyData = useMemo(() =>
    buildDailyChartData(filteredData, filteredGoals, availableKeys, chartKpi, start, goalsEnd, chartBUs),
    [filteredData, filteredGoals, availableKeys, chartKpi, start, goalsEnd, chartBUs]);

  // ── To-Go table ───────────────────────────────────────────────────────────

  const toGoRows = useMemo(() =>
    buildToGoTable(filteredData, filteredGoals, availableKeys, start, maxGoalDate || end, chartBUs, today),
    [filteredData, filteredGoals, availableKeys, start, end, maxGoalDate, chartBUs, today]);

  // ── Projections & pace ────────────────────────────────────────────────────

  const lastRealDataDate = useMemo(() => {
    const df = findField(availableKeys, 'data', 'date', 'created_at');
    if (!df || !filteredData.length) return today;
    const dates = filteredData.map(d => parseLocalDate(d[df])).filter(d => !isNaN(d.getTime()));
    if (!dates.length) return today;
    return new Date(Math.max(...dates.map(d => d.getTime()))).toISOString().split('T')[0];
  }, [filteredData, availableKeys, today]);
      ...
      <div style={{ ...panel, padding: 16 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'flex-end' }}>
          ...
          {produtoOptions.length > 0 && (
            <div>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#a0a0bc', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Produto / BU</label>
              <select value={filterProduto} onChange={e => setFilterProduto(e.target.value)}
                style={{ padding: '6px 10px', fontSize: 13, border: '1px solid var(--border-default)', borderRadius: 8, background: 'var(--bg-elevated)', color: 'var(--text-primary)', outline: 'none', minWidth: 160 }}>
              ...
              {filterProduto !== 'all' && (
                <div style={{ fontSize: 10, color: 'var(--accent)', marginTop: 3, fontWeight: 600 }}>
                  Metas e projeções filtradas para {filterProduto}
                </div>
              )}
            </div>
          )}
