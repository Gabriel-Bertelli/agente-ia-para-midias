...
function buildCourseCACMap(
  ...
  for (const row of data) {
    ...
    // Rows with no course at all: attribute to a synthetic 'sem_curso' bucket
    // so campaigns without course still get a valid CAC score
    if (courseNameCampanha && courseNameCaptacao) {
      const campCourse = String(row[courseNameCampanha] ?? '').trim();
      const captCourse = String(row[courseNameCaptacao] ?? '').trim();
      if ((!campCourse || campCourse === '(not set)') && (!captCourse || captCourse === '(not set)')) {
        const inv = safeNum(row[invField]);
        const mat = safeNum(row[matField]);
        const b = ensure('sem_curso');
        b.inv_total += inv;
        b.mat_total += mat;
        if (isPaid) { b.inv_paid += inv; b.mat_paid += mat; }
      }
    }
  }
...
function resolveEffectiveMaxChange(
  ...
  if (courses.length === 0) {
    // No course at all — use neutral teto so the campaign can still participate
    // in allocation via its own CAC (campaign-level water-filling)
    return SCORE_CONFIG['neutral'].maxChange;
  }
  if (courses.length === 1) {
    const c     = courseMap.get(courses[0]);
    // 'sem_curso' is a legitimate bucket — if it has a CAC/score, use it;
    // otherwise fall back to neutral (not 0%) so the campaign isn't frozen
    const score = c ? (cacMode === 'paid' ? c.score_paid : c.score_total) : 'neutral';
    return SCORE_CONFIG[score === 'no_data' ? 'neutral' : score].maxChange;
  }
  ...
  for (const course of courses) {
    ...
    let score: CourseScore = c ? (cacMode === 'paid' ? c.score_paid : c.score_total) : 'neutral';
    if (score === 'no_data') score = 'neutral';
    weightedMax += SCORE_CONFIG[score].maxChange * share;
    totalShare  += share;
  }
  return totalShare > 0 ? weightedMax / totalShare : SCORE_CONFIG['neutral'].maxChange;
}
...
function resolveScore(
  ...
  if (courses.length === 0) return 'neutral';
  if (courses.length === 1) {
    const c = courseMap.get(courses[0]);
    if (!c) return 'neutral';
    const raw = cacMode === 'paid' ? c.score_paid : c.score_total;
    return raw === 'no_data' ? 'neutral' : raw;
  }
  ...
  let bestScore: CourseScore = 'neutral';
  let bestDist = Infinity;
  for (const [score, cfg] of entries) {
    if (score === 'no_data') continue;
...
function allocate(
  ...
  // Campanhas sem CAC: maintain current when increasing, floor when decreasing.
  // sem_curso campaigns get neutral teto (±35%) so they have valid min/max bounds,
  // but without a measurable CAC, budget flows preferentially to campaigns we CAN measure.
  // Their inv_max is respected if the CAC-based campaigns saturate their own ceilings.
  ...
  let budgetForCac = targetTotal - fixedTotal;
  ...
  // If CAC-based campaigns couldn't absorb all budget, redistribute overflow to no-CAC campaigns
  const cacAllocTotal   = cacAllocs.reduce((s, a) => s + a.inv_suggested, 0);
  const budgetOverflow  = budgetForCac - cacAllocTotal;

  if (budgetOverflow > 1 && noCacAllocs.length > 0 && isIncreasing) {
    // Distribute proportionally up to each no-CAC campaign's inv_max
    const noCacHeadroom = noCacAllocs.map(a => a.inv_max - a.inv_suggested);
    const totalHeadroom = noCacHeadroom.reduce((s, v) => s + Math.max(0, v), 0);
    if (totalHeadroom > 0) {
      let toDistribute = Math.min(budgetOverflow, totalHeadroom);
      for (let i = 0; i < noCacAllocs.length && toDistribute > 0.01; i++) {
        const headroom = Math.max(0, noCacHeadroom[i]);
        const share    = headroom / totalHeadroom;
        const extra    = Math.min(share * budgetOverflow, headroom);
        noCacAllocs[i].inv_suggested = Math.round((noCacAllocs[i].inv_suggested + extra) * 100) / 100;
        noCacAllocs[i].delta_abs     = noCacAllocs[i].inv_suggested - noCacAllocs[i].inv_current;
        noCacAllocs[i].delta_pct     = noCacAllocs[i].inv_current > 0
          ? (noCacAllocs[i].delta_abs / noCacAllocs[i].inv_current) * 100 : 0;
        noCacAllocs[i].capped        = Math.abs(noCacAllocs[i].inv_suggested - noCacAllocs[i].inv_max) < 0.5;
        noCacAllocs[i].cappedBy      = noCacAllocs[i].capped ? 'max' : null;
        toDistribute -= extra;
      }
    }
  }
...
export function InvestmentSuggestion({ data }: { data: any[] }) {
  ...
  // Build friendly short names for campaigns: "platform · tipo_campanha"
  // e.g. "pucpr_prf_aon_cadastro-site_pos-grad_inst-comunicacao-digital" → "Facebook · cadastro site"
  const platformField = useMemo(() => findField(keys, 'platform'), [keys]);

  const campaignNicknames = useMemo(() => {
    const map = new Map<string, string>();
    if (!campaignField) return map;
    for (const row of filteredData) {
      const campaign = String(row[campaignField] ?? '').trim();
      if (!campaign || map.has(campaign)) continue;
      const platform = platformField ? String(row[platformField] ?? '').trim() : '';
      const tipo     = tipoCampanhaField ? String(row[tipoCampanhaField] ?? '').trim() : '';
      // Build nickname: "Platform · tipo" or just tipo, or abbreviated campaign name
      const parts: string[] = [];
      if (platform) parts.push(platform);
      if (tipo && tipo.toLowerCase() !== platform.toLowerCase()) parts.push(tipo);
      if (parts.length > 0) {
        map.set(campaign, parts.join(' · '));
      } else {
        // Fallback: take last meaningful segment of campaign_name
        const segments = campaign.replace(/_/g, ' ').replace(/-/g, ' ').split(/\s+/);
        const meaningful = segments.filter(s => s.length > 3).slice(-3);
        map.set(campaign, meaningful.join(' ') || campaign.slice(0, 40));
      }
    }
    return map;
  }, [filteredData, campaignField, platformField, tipoCampanhaField]);
  ...
  const campaignCourseShare = useMemo(() => {
    ...
    if (!campaignField || !invField) return shareMap;
    for (const row of filteredData) {
      const campaign = String(row[campaignField] ?? '').trim();
      if (!campaign) continue;
      let course = courseNameCampanha ? String(row[courseNameCampanha] ?? '').trim() : '';
      // Normalize missing/empty/not-set course names to 'sem_curso'
      if (!course || course === '(not set)' || course.toLowerCase() === 'not set') {
        course = 'sem_curso';
      }
      const inv = safeNum(row[invField]);
              ...
              <div key={alert.course} className="text-xs text-blue-800 flex items-start gap-2">
                ...
                <span>
                  ...
                  {alert.cappedCampaigns.slice(0, 3).map(c => (
                    <code key={c} className="bg-blue-100 px-1 rounded mx-0.5" title={c}>{campaignNicknames.get(c) || c}</code>
                  ))}
