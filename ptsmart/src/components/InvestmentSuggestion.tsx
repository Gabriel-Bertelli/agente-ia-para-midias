...
interface CampaignMetrics {
  campaign: string;
  inv_current: number; // investimento médio diário (7d, fallback 30d)
  inv_30d_total: number; // total bruto 30d — usado para filtro de campanha ativa
  cac_60d: number | null;
  cac_30d: number | null;
  cac_14d: number | null;
  cac_7d: number | null;
...
function allocate(campaigns: CampaignMetrics[], targetTotal: number): SuggestionResult {
  ...
  const reservedNoData = withoutCac.reduce((s, c) => s + Math.max(c.inv_current, 1) * (1 - MAX_CHANGE), 0);
  ...
  while (iterations < MAX_ITER) {
    ...
    const cappedFlags = withCac.map((c, i) => {
      // Garante base mínima de R para evitar variação % infinita em campanhas com inv baixo
      const base = Math.max(c.inv_current, 1);
      const min = base * (1 - MAX_CHANGE);
      const max = base * (1 + MAX_CHANGE);
      if (firstPass[i] < min) {
  ...
  const allocations: AllocationResult[] = [
    ...withCac.map((c, i) => {
      ...
      return {
        ...
        inv_suggested,
        inv_min: Math.round(base * (1 - MAX_CHANGE) * 100) / 100,
        inv_max: Math.round(base * (1 + MAX_CHANGE) * 100) / 100,
        delta_pct,
        delta_abs,
        capped: capped[i],
      };
    ...
    ...withoutCac.map(c => {
      const base = Math.max(c.inv_current, 1);
      const inv_suggested = Math.round(base * (1 - MAX_CHANGE) * 100) / 100;
      const delta_abs = inv_suggested - c.inv_current;
      const delta_pct = c.inv_current > 0 ? (delta_abs / c.inv_current) * 100 : 0;
      return {
        campaign: c.campaign,
...
export function InvestmentSuggestion({ data }: { data: any[] }) {
  ...
  // Calcular métricas por campanha — filtrado: só campanhas com investimento nos últimos 30 dias
  const campaignMetrics = useMemo((): CampaignMetrics[] => {
    ...
    return campaigns
      .map(campaign => {
        ...
        // Investimento atual: média diária 7d. Se zero (pausa recente), usa média 30d
        ...
        const inv_30d_total = inv_30d_daily * 30; // usado apenas para filtro de ativa
        ...
        return { campaign, inv_current, inv_30d_total, cac_60d, cac_30d, cac_14d, cac_7d, cac_weighted, mat_60d, mat_7d };
      })
      // Exibe apenas campanhas com investimento nos últimos 30 dias (ativas)
