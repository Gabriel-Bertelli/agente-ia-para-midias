...
/**
 * Hook para usar Web Worker para agregação de campanhas/cursos
  ...
  useEffect(() => {
    try {
      ...
      const handleMessage = (event: MessageEvent) => {
        ...
        if (type === 'aggregate-campaigns-result') {
...
/**
 * Hook simples que verifica se worker está disponível
 */
export function useWorkerAvailable(): boolean {
  const [available, setAvailable] = useState(false);

  useEffect(() => {
    try {
      // Test if Worker is supported
